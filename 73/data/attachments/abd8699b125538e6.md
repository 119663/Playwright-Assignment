...
Locator: getByText('Logged in as Cody')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Logged in as Cody')
...
```ts
  ...
  43  |     await fillSignupForm.continueButton.click();
  44  |     // await page.locator('//a[@data-qa="continue-button"]').click();
  45  | 
> 46  |     await expect(page.getByText('Logged in as ' + firstName)).toBeVisible();
      |                                                               ^ Error: expect(locator).toBeVisible() failed
  47  | 
  48  | 
